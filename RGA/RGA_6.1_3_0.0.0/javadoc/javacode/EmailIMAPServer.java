 ...
 */
public class EmailIMAPServer extends Feed {

    /** The Constant CLASS_ID. */
    private static final String CLASS_ID = "EmailIMAPServer";
     ...
     */
    EmailIMAPServer(String server, String userID, String password,
    ...
    public static String validate(String address, String userID, String password, String serverParams) {
        ...
        props = EmailIMAPServer.setSSLProperty(setSSLProperty(props));
    ...
    Folder getInboxFolder() throws Exception {
    	...
    	String propStr = "mail.imap.ssl.enable=true";
        if (inboxFolder != null) {
            return inboxFolder;
        }
        ...
        String SSL_Property = props.getProperty("mail.imap.ssl.enable");
        if (SSL_Property != null && SSL_Property.equals("true")) {
            props = EmailIMAPServer.setSSLProperty(props);
        }
        // Get the instance
        popSession = Session.getDefaultInstance(props, null);
        popStore = popSession.getStore("imap");
    ...
    FeedXtraxnStats getMessages(List<ChannelFeedData> messages, Map<String, String> feedTimeStamps)
        ...
        if (!inboxFolder.isOpen()) {
            folder.open(Folder.READ_WRITE); // added by SumitL
        }
        ...
        if (inboxFolder != null) {
            ...
            try {
                ...
                if (inboxFolder.isOpen()) { // Ensure an open folder
                    ...
                    Logger.info(CLASS_ID, "server address for " + userID + " "
                            + this.address, null);
                    if(this.address.equalsIgnoreCase("imap-mail.outlook.com")){
                    		Calendar cal = Calendar.getInstance();
                    		cal.setTimeInMillis(lastFeedTimestamp);
                    		newer = new SentDateTerm(ComparisonTerm.GT, cal.getTime());
                    }
                            ...
                            try {
                                ...
                                if (feedTimestamp > lastFeedTimestamp) {
                                    ...
                                    Logger.info( CLASS_ID, "---------------------------------:"
                                    		+ i + "; start =" + (Calendar.getInstance().getTime().toString()), null);
                                    ...
                                    Logger.info( CLASS_ID, "---------------------------------:"
                                            + i + "; end =" + (Calendar.getInstance().getTime().toString()), null);
                                    stats.incrementedAccepted();
                                    if (maxFeedTimestamp < feedTimestamp) {
                                        maxFeedTimestamp = feedTimestamp;
                                    }
                                    Logger.debug( CLASS_ID, "iter:" + i + "; time taken for processing a valid feed="
                                ...
                                Logger.error( CLASS_ID, "Error reading msg with Subj : '"
